...
function svgMapWrapper(svgPanZoom) {
  ...
  // Custom tooltip handling (keep this part if needed)
  ...
  if (this.options.hideFlag === false) {
    ...
    var flagContainer = this.createElement(
      'div',
      'svgMap-tooltip-flag-container svgMap-tooltip-flag-container-' + this.options.flagType,
  ...
  if (this.options.data.values[countryID]) {
    // Country has data, show table with headers
    var table = this.createElement('table', 'svgMap-tooltip-table', tooltipContent);
    
    // Create table header
    var thead = this.createElement('thead', '', table);
    var headerRow = this.createElement('tr', '', thead);
    ['VISA Direct', 'VISA', 'Mastercared', 'Apple Pay', 'Google Pay'].forEach(function(header) {
      this.createElement('th', '', headerRow).innerHTML = header;
    }, this);

    // Create table body
   // Create table body
var tbody = this.createElement('tbody', '', table);
var countryData = this.options.data.values[countryID];
if (Array.isArray(countryData.methods)) {
    countryData.methods.forEach(function(method) {
        var dataRow = this.createElement('tr', '', tbody);
        ['visadirect', 'visa', 'mastercard', 'applepay', 'googlepay'].forEach(function(key) {
            var cell = this.createElement('td', '', dataRow);
            var value = method[key];
            var format = this.options.data.data[key].format;
            
            if (value === "TRUE" || value === "FALSE") {
                cell.innerHTML = value === "TRUE" ? "&#x2714;" : "&#x2716;";
                cell.style.textAlign = 'center';
                cell.style.fontSize = '1.2em';
            } else {
                cell.innerHTML = format.replace('{0}', value);
            }
    ...
    // Fallback for countries with single method
    var dataRow = this.createElement('tr', '', tbody);
    ['visadirect', 'visa', 'mastercard', 'applepay', 'googlepay'].forEach(function(key) {
        var cell = this.createElement('td', '', dataRow);
    ...
    // Country is not supported, show message without table headers
    tooltipContent.innerHTML = this.options.noDataText;
  }
